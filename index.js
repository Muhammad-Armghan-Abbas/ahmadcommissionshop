    ...
    sellersData.forEach(function (seller) {
        ...
        seller.sales.forEach(function (sale, index) {
            ...
            entryRow.innerHTML = `
                ...
                <td style="border:1px solid black"> ${sale.Bill.toFixed(0)}</td>
                <td style="border:1px solid black"> ${sale.Price.toFixed(0)}</td>
                <td style="border:1px solid black">${sale.Net_Weight.toFixed(2)} </td>
                <td style="border:1px solid black">${sale.Kat.toFixed(2)}</td>
                <td style="border:1px solid black">${sale.Weight}</td>
...
function generateBill() {
    ...
    for (let buyerName in buyers) {
        ...
        billWindow.document.write(`
        <tr>
        <td colspan"6" style="font-size:16px;height:21px;"></td>
        </tr>
            <tr style="border:1px solid black">
                <td colspan="" style="width:21%;text-align:left;border:1px solid black">
                    <input style="background: transparent;text-align:center;font-size:16px;border:transparent;width:100%;padding: 0;margin:0 0 0 7px;" type="number" id="additionalAmount_${buyerName}" placeholder="Enter rent amount" onchange="updateTotalBill('${buyerName}')" onchange="updateBill('${buyerName}')">
                </td>
                ...
                <td>
                <input style="background: transparent; text-align: center; font-size: 16px; border: transparent;min-width:60px;width: 100%;padding: 0;margin:0 0 0 7px;" type="number" id="remainingAmount_${buyerName}" oninput="updateBill('${buyerName}')" placeholder="Enter remaining amount">
                </td>
                </tr>
                <tr style="text-align:center">
                <td colspan="1" >
                <span id="totalBill_${buyerName}">${buyer.totalBill}</span>
                </td>
                </tr>
                </table>
                </td>
            ...
            <td><b>: موجودہ بل </b></td>
                ...
                <td colspan="1" style="border:1px solid black">
                    <span id="totalWeight">${buyer.totalWeight}</span> 
                </td>
        ...
        billWindow.document.write(`
            <tr style="border:1px solid black">
                <td colspan="1" style="border:1px solid black">
                      <span id="totalBill1_${buyerName}">${buyer.totalBill}</span>
                </td>
                <td colspan="1" style="text-align:left;border:1px solid black">
                   <b> :بل</b>
                </td>
                <td colspan="1" style="border:2px solid black">
                <span id="total_${buyerName}">0</span>
            </td>
            <td colspan="1" style="border:2px solid black;text-align:left">
                <b>: قل رقم</b>
            </td>
            <td colspan="2"></td>
            </tr>
    ...
    let newTotalBill;
    billWindow.updateBill = function (buyerName) {
        let remainingInput = billWindow.document.getElementById(`remainingAmount_${buyerName}`);
        let remaining = parseFloat(remainingInput.value) || 0;
        
        // Update the total bill cell with the remaining amount
        let totalBillCell = billWindow.document.getElementById(`total_${buyerName}`);
        totalBillCell.innerText = remaining.toFixed(0);
        
        // Calculate and update the new total bill for the buyer
        let totalBillSpan1 = billWindow.document.getElementById(`total_${buyerName}`);
        ...
        let newTotalBill = currentTotalBill + additionalAmount + remaining;
        
        // Update the displayed total bill for the buyer
        totalBillSpan1.innerText = newTotalBill.toFixed(0);
    };
    ...
    billWindow.updateTotalBill = function (buyerName) {
        ...
        let totalBillSpan1 = billWindow.document.getElementById(`totalBill1_${buyerName}`);
        let additionalAmount = parseFloat(additionalAmountInput.value) || 0;

        // Use the original total bill without additional additions
        let currentTotalBill = parseFloat(originalTotalBill[buyerName]);
        newTotalBill = currentTotalBill + additionalAmount;

        totalBillSpan.innerText = newTotalBill.toFixed(0);
        totalBillSpan1.innerText = newTotalBill.toFixed(0);
    };
...
function generateSupplierTableOnSinglePage(billWindow, sellerName) {
    ...
    billWindow.document.write(`<div class="buyer-section">`);
    billWindow.document.write(`<h1 style="margin:10px;text-align:center;">احمد کمیشن شاپ</h1>`);
    billWindow.document.write(`<h2 style="text-align:center;"><p style="font-weight: 600; font-size: 16px; margin: -56px 0px 0px 12px; display: inline; float: left;"> مجاہد حسین <br>03346890792<br>03400634331 </p> <p style="font-size: 14px;font-weight: 300;background-color: black;color:white;border: transparent; width: 19%;margin: auto;border-radius: 16px;">آڑھت لکڑ منڈی لیاقت پور</p></h2>`);
    ...
    let totalBill = seller.sales.reduce((acc, sale) => acc + sale.Bill, 0).toFixed(0);
    ...
    billWindow.document.write(`
        ...
        <td colspan="2" style="border:1px solid black"></td>
            <td colspan="1" style="border:1px solid black">
            <span id="tolai_${sellerName}"><b>${(seller.sales.length + 1) * 20}</b></span>
            </td>
            <td colspan="1" style="border:1px solid black">
                <b> :تولائ</b>
            </td>
    ...
    billWindow.document.write(`
        <tr style="border:1px solid black">
            ...
            <td colspan="1" style="border:1px solid black">
            <span><b><input style="border:transparent;background-color: transparent;font-size:16px;font-weight:700; text-align:unset; width:80px;height:18px; margin:0px;" type="number" id="paledari_${sellerName}" onchange="updatePaledariAmount('${sellerName}')"></b></span>
            </td>
            <td colspan="1" style="border:1px solid black">
                <b> :پلے داری</b>
            </td>
    ...
    billWindow.document.write(`
        <tr style="border:1px solid black">
            <td colspan="4" style="border:1px solid black"></td>
            <td colspan="1" style="border:1px solid black">
            <span><b><input style="border:transparent;background-color: transparent;font-size:16px;font-weight:700; width:80px;text-align:unset ; height:18px;margin:0px;" type="number" id="moche_${sellerName}" onchange="updateMocheAmount('${sellerName}')"></b></span>
            </td>
            <td colspan="1" style="border:1px solid black">
                <b> :مو چھے</b>
            </td>
    ...
    const script = `
    <script>
        let paledariAmount = 0;
        let mocheAmount = 0;
    
        function updatePaledariAmount(sellerName) {
            let paledariInput = document.getElementById('paledari_' + sellerName);
            paledariAmount = parseFloat(paledariInput.value) || 0;
            updateTotalExpenses(sellerName);
            paledariInput.value = (paledariAmount * 3).toFixed(0);
        }
    
        function updateMocheAmount(sellerName) {
            let mocheInput = document.getElementById('moche_' + sellerName);
            mocheAmount = parseFloat(mocheInput.value) || 0;
            updateTotalExpenses(sellerName);
            mocheInput.value = (mocheAmount * 4).toFixed(0);
        }
    
        function updateTotalExpenses(sellerName) {
            let Tolai=parseFloat(document.getElementById('tolai_' + sellerName).innerText);
            let commissionAmount = (parseFloat(document.getElementById('totalBill_' + sellerName).innerText) * 0.06);
            let totalExpenses = ((paledariAmount * 3) + (mocheAmount * 4) + Tolai + commissionAmount + 50).toFixed(0);
    ...
    // Now, use the script in the appropriate location in your HTML
